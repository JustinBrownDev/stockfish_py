...
make strip
mv stockf.exe ../../stockfish_${arch_cpu}.exe
make clean
cd
```
</details>
...
```bash
$ mkdir -p ~/repos/official-stockf
$ cd ~/repos/official-stockf
...
```bash
$ file stockf
stockf: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, with debug_info, not stripped
```
...
aarch64-linux-android-strip stockf
```
...
```bash
$ file stockf
stockf: ELF 64-bit LSB shared object, ARM aarch64, version 1 (SYSV), dynamically linked, interpreter /system/bin/linker64, stripped
```

It is a good idea to rename the binary:

```bash
$ mv stockf stockfish_DEV_armv8
```
...
mv stockf.exe ../../stockf-x64${1}.exe
...
make strip COMP=mingw
mv stockf.exe ../../stockf-x64${1}-pgo.exe
make clean COMP=mingw
}
...
qemu-aarch64 stockf compiler
qemu-aarch64 stockf bench
...
# test: qemu's magic at work
qemu-arm stockf compiler
qemu-arm stockf bench
